...
def make_handlers(bk: str, label: str):
    ...
    async def start_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # Register karo (admin ho ya normal user)
        add_user(bk, uid, user.first_name, user.username or "")

        # Start message — sabko (user + admin dono ko)
        start_kb = None
        if cfg.START_BUTTONS:
            start_kb = InlineKeyboardMarkup(
                [[InlineKeyboardButton(b["text"], url=b["url"])]
                 for b in cfg.START_BUTTONS]
            )

        if cfg.START_IMG:
            await update.message.reply_photo(
                photo=cfg.START_IMG,
                caption=cfg.START_TEXT,
                parse_mode=ParseMode.HTML,
    ...
    # ── /admin — sirf owner/admin ke liye panel ──
    ...
    return start_cmd, admin_cmd, bc_cmd, cb, msg_hdl, jr_hdl
...
def build_app(token: str, bk: str, label: str) -> Application:
    sc, ac, bcc, cb, mh, jr = make_handlers(bk, label)
    app = Application.builder().token(token).build()
    app.add_handler(CommandHandler("start",     sc))
    app.add_handler(CommandHandler("admin",     ac))
